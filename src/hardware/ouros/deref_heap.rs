...
impl DrfHeap {
    ...
    pub fn port_a_ready(&self) -> bool {
        let out_clear = !self.holder_out.0 || self.output_fire();
            ...
            match *self.stm.value() {
                ...
                Stm::OPb => {
                    if !self.heap_mem.dout_a().exist {
                        ...
                        true
                    } else if self.output_fire() || !self.holder_out.0 {
        ...
        let local_used = match *self.stm.value() {
            ...
            Stm::OPb => {
                if !self.heap_mem.dout_a().exist {
                    ...
                    && (self.output_fire() || !self.holder_out.0)
                {
                    match self.holder_in.value().load[1] {
                        Atom::INT(_) => !*self.working_heap.dout_a(),
                        _ => true,
                            ...
                            Atom::INT(_) => match self.input.port_a_bits.load[2] {
                                Atom::PTR(p) => {
                                    ...
                                    self.demand_heap.write_a(
                                        p,
                                        !(!is_int(&self.input.port_a_bits.load[1])
    ...
    fn update_local(&mut self) {
            ...
            Stm::OPa => {
                ...
                if !exist {
                    ...
                    match self.holder_in.value().load[2] {
                        ...
                        Atom::INT(_) => {
                            // store the demander
                            match self.thread_stack[self.holder_in.value().stack_idx as usize].top()
                            {
                                Some(addr) => self.heap_mem.write_b(
                                    *addr,
                                    HeapCell {
                ...
                if !exist {
                    ...
                    match self.holder_in.value().load[1] {
                        Atom::INT(_) => {
                            ...
                            addr = *self.thread_stack[self.holder_in.value().stack_idx as usize]
                                .top()
                                .unwrap();
                        }
                        _ => {
                        ...
                        _ => {
                            ...
                            match self.pick_stack() {
                                Some(stk_idx) => {
                                    ...
                                    self.heap_mem.write_b(
                                        *self.thread_stack
                                            [self.holder_in.value().stack_idx as usize]
                                            .second()
                                            .unwrap(),
                                        HeapCell {
                                            exist: true,
                    ...
                    // keep reading b
                    panic!(
                        "funny thing, out_ready: {}, out_valid: {}",
                        self.input.out_main_ready,
                        self.out_main_valid()
                    );
                    match self.holder_in.value().load[2] {
        ...
        if fire(self.input.port_b_valid, self.port_b_ready()) {
            ...
            if self.heap_mem.input.port_b.is_write {
                panic!("port_b: strange overwrite!");
            }
